...
interface FormErrors extends IPlayerData {
    form: string;
}
interface IProps extends RouteComponentProps<{ id: string }> { }

function PlayerEdit(props: IProps) {
    const classes = useStyles();
    const { history, match, location } = props;
    const [playerData, setPlayerData] = useState({} as IPlayerData)
    ...
    const getPlayerData = async () => {
        try {
            const doc = await db
                ...
                .doc(match.params.id)
                .get();
            const data = doc.data() as IPlayerData;
            data.id = doc.id;
            setPlayerData(data);
            setValues(data);
    ...
    const redirectToTargetPage = () => {
        let prevPath = `/player/detail/${match.params.id}`;
